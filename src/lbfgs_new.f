...
c                                                   from 'cauchy').
c      write(*,*) 'z before cmprlb:',z(58)
      call cmprlb(n,m,x,g,ws,wy,sy,wt,z,r,wa,index,
     +           theta,col,head,nfree,cnstnd,info)
c      write(*,*) 'z after cmprlb:',z(58)
...
c     **********
      ...
      parameter        (one=1.0d0,zero=0.0d0,big=1.0d10) 
      double precision ftol,gtol,xtol
      parameter        (ftol=1.0d-3,gtol=0.9d0,xtol=0.1d0)
      double precision xx !cwp for quick and dirty fix of stability      
...
c      write(*,*) 'STP:',stp
	 ...
	 ifun = ifun + 1
         ...
         if (stp .eq. one) then
            ...
            do 41 i = 1, n
c               xx=x(i) !cwp store x(i) tempararily
	       x(i) = stp*d(i) + t(i)
c               if(abs(xx-x(i)).ge.0.5) then !cwp if the jump too big
c                   task = 'RESTART' ! cwp restart the minimization
c                   x(i)=xx ! cwp go back to original config.
c                   return ! cwp go back
c               endif
  41        continue
         endif
      else
...
c      write(*,*) 'after dcsrch:',x(58),x(59),x(60),stp,d(58),t(58) !cwp for debugging
