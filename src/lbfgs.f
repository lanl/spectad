...
C     ----------------------------------------------------
 ...
 170  W(I)=G(I)
 172  CONTINUE
      WRITE(*,*) 'before MC',X(25),X(26),X(27) !cwp debug
      CALL MCSRCH(N,X,F,G,W(ISPT+POINT*N+1),STP,FTOL,
     *            XTOL,MAXFEV,INFO,NFEV,DIAG)
      WRITE(*,*) 'after MC',X(25),X(26),X(27) !cwp debug
      IF (INFO .EQ. -1) THEN
        IFLAG=1
        RETURN
      ENDIF
...
c
      double precision dx(*),dy(*),da ! was dx(1),dy(1) cwp?????
...
c     jack dongarra, linpack, 3/11/78.
c
      double precision dx(*),dy(*),dtemp ! was dx(1) dy(1) cwp ?????
       integer i,incx,incy,ix,iy,m,mp1,n
c
      ddot = 0.0d0
      dtemp = 0.0d0
